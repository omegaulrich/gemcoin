...
#if defined(NDEBUG)
# error "Gemcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x191ff4f20841dbfee41700a08dfb73ecfb268071fda0c6fd49282b4875b4a7b8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gemcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gemcoin Signed Message:\n";
...
{
    // Gemcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 15000); // Gemcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Gemcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Gemcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gemcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gemcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4b959a9fbb276f7eff9bc87e30aa76a9ffcb21836a80eb89954f41fb8cf0fd60");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "$44 BILLION TECH DEAL US/CHINA";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048f703c27483fbadb8be36873d3126782955660bbff1f0272dfeb23e2ec79f78f6c9ec4a25d63d22e8a6a694a6b19a3ffcebe73915109dae3e8850b35b73b3fb7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524590537;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085806906;

        if (fTestNet)
        {
            block.nTime    = 1524590483;
            block.nNonce   = 385643272;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcb3ff2128e3e12fb77a73fc33b90bc2a6dc7785322df4964a4c6ce023fc79bd4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xdf, 0xc5, 0xd6, 0xbb }; // Gemcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GemcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GemcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GemcoinMiner(CWallet *pwallet)
{
    printf("GemcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gemcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GemcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GemcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GemcoinMiner, pwallet));
}
